<template>
    <div class="fixed top-0 left-0 w-screen h-[60px] 
                flex place-content-center bg-bg1 z-20">
        <div class="flex justify-between items-center w-pc">

            <div class="grid grid-cols-2 w-56">
                <router-link to="/">
                ...
                <span class="tracking-[3px] border-l-2 border-l-style4 pl-3 select-none">
                    <span v-if="router.currentRoute.value.path == '/uploadpage'">簽署新文件</span>
                    <span v-else-if="router.currentRoute.value.path.substring(0, 12) == '/pdfEditpage'">編輯文件</span>
                </span>
            </div>

            <div class="flex justify-center item-center gap-x-6 font-bold tracking-[3.5px]">
                ...
                <div class="flex justify-center items-center
                            ...
                            bg-top hover:bg-bottom bg-bgtrans1 bg-clip-text
                            text-transparent font-notosans-light cursor-pointer
                            transition-color duration-200 select-none" @click="toggleloginmodal">登入
                </div>
            </div>
        </div>
...
const toggleloginmodal = _.throttle(() => loginmodal_check.value = !loginmodal_check.value, 800,)
</script>